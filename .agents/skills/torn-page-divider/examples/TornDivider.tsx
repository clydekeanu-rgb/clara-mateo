...
export interface TornDividerProps {
  ...
  toColor?: string;
  /** Invert horizontally for natural variety across multiple dividers */
  flipped?: boolean;
  /** Height in pixels (defaults to 80px) */
  height?: number;
...
export const TornDivider: React.FC<TornDividerProps> = ({
  ...
  // SSR-safe unique filter identifier to prevent collision between multiple dividers
  const rawId = useId();
  ...
  const isNightBottom =
    toColor === '#0D1512' ||
    toColor === '#000000' ||
    toColor.toLowerCase().includes('0d1512') ||
    toColor.toLowerCase().includes('night');
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 2. Exposed Cotton Pulp Deckle Rim: Sits directly under the rim and slightly protrudes */}
        <path
          d={tearLine}
          fill="none"
          stroke={pulpColor}
